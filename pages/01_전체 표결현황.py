...
bill_name_options = ["전체"] + sorted(df.drop_duplicates(subset=["의안번호"])["의안명"].tolist())
selected_bill_name = st.sidebar.selectbox("의안명 선택 (개별 조회)", bill_name_options)
if selected_bill_name != "전체":
    df = df[df["의안명"] == selected_bill_name]
...
st.subheader("월별 표결 의안 수")
by_month = (
    df.dropna(subset=["표결일자"])
    .assign(표결월=df["표결일자"].dt.to_period("M").astype(str))
    .groupby("표결월")["의안번호"].nunique().reset_index()
)
by_month.columns = ["표결월", "의안수"]
by_month = by_month.sort_values("표결월")
st.plotly_chart(
    px.bar(by_month, x="표결월", y="의안수", labels={"표결월": "월", "의안수": "의안 수"}),
...
st.plotly_chart(
    px.pie(result_dist, names="표결결과", values="건수", hole=0.4, color="표결결과",
