...
st.sidebar.subheader("여야 매핑 설정 (갈등도 계산용)")
ruling_input = st.sidebar.text_input("여당", "국민의힘")
opposition_input = st.sidebar.text_input("주요 야당", "더불어민주당")
...
if use_sample:
    ...
    vote_raw, _err = fetch_vote_info_single(selected_bill_id, age_num)
...
conflict_df = compute_bipartisan_conflict(bill_vote_df, ruling_parties, opposition_parties)
if not conflict_df.empty:
    row = conflict_df.iloc[0]
    c5, c6 = st.columns(2)
    c5.metric("여야 갈등도", f"{row['conflict_index']:.2f}")
    c6.metric("초당적 합의도", f"{row['bipartisan_agreement_index']:.2f}")
    st.caption("갈등도 = abs(여당 찬성률 - 야당 찬성률). 여야 매핑은 사이드바 설정을 따름 (공식 자료 아님).")
else:
