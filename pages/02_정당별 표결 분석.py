...
if valid.empty:
    ...
    st.plotly_chart(
        px.bar(party_dist, x="정당명", y="ratio", color="표결결과", barmode="group",
               labels={"정당명": "정당", "ratio": "비율", "표결결과": "표결결과"},
               text=party_dist["ratio"].apply(lambda x: f"{x:.0%}"),
               color_discrete_map=VOTE_COLOR_MAP),
        use_container_width=True,
    )
    st.caption("각 정당별로 찬성/반대/기권 막대를 나란히 표시합니다. 비율은 정당 내 표결 건수를 100%로 둔 값입니다.")
...
if not conflict_df.empty:
    ...
    st.markdown(
        "📌 **계산식**\n"
        "- 여당 찬성률 = 더불어민주당 소속 의원 중 찬성한 비율\n"
        "- 야당 찬성률 = 더불어민주당을 제외한 나머지 정당 전체 의원 중 찬성한 비율\n"
        "- 갈등도 = abs(여당 찬성률 − 야당 찬성률) → 0에 가까울수록 여야가 비슷하게 투표, 1에 가까울수록 정반대\n"
        "- 초당적 합의도 = 1 − 갈등도\n"
        "- 22대 국회 기준 더불어민주당을 여당, 나머지 정당 전체를 야당으로 고정하여 계산한 것이며 공식 당론 자료가 아닙니다."
    )
else:
